...
node('amd64 && docker') {
    ...
    sh 'docker run --rm -e GOARCH=${GOARCH} -e CGO_ENABLED=${CGO_ENABLED} --privileged -u "r" -v /etc/passwd:/etc/passwd:ro -v /etc/group:/etc/group:ro -v "$PWD":/usr/src/myapp -w /usr/src/myapp test /bin/bash -c "chmod +x docker_build; chmod +x test; chmod +x build; /usr/src/myapp/docker_entry.sh"'
}
